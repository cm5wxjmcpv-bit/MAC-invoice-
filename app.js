...
function makePdf(button = null) {
  ...
  const action = () => {
    ...
    y += 10;
    doc.setFontSize(12);
    doc.text(`Invoice #${invoice.invoiceNumber || "Draft"}`, 14, y);

    y += 7;
    doc.text(`Date: ${invoice.date || today()}`, 14, y);

    y += 7;
    doc.text(`Status: ${(invoice.status || "unpaid").toUpperCase()}`, 14, y);
    ...
    [invoice.customerName, invoice.customerPhone, invoice.customerEmail, invoice.customerAddress]
      .filter(Boolean)
      .forEach((line) => {
        y += 6;
    ...
    y += 8;
    doc.setFontSize(14);
    doc.text(`Invoice Total: ${money(invoice.total)}`, 120, y);
    ...
    if (invoice.notes) {
      ...
      y += 6;
      doc.setFontSize(10);
      doc.text(invoiceNoteLines, 14, y);
    }
